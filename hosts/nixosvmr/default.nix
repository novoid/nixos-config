...

{
  ...
  networking.hostName = "nixosvmr"; # Define your hostname.
...
#    services.blueman-applet.enable = config.hardware.bluetooth.enable == true;
  ...
  # FIXXME: could be beautified by referring to $HOSTNAME (somehow in a Nix-way) instead of "nixosvm(s)":
#  home.file.".tmuxp/nixosvmr.yaml".source = ../../assets/.tmuxp/nixosvm.yaml; # I usually start my main tmux session via: tmuxp load $HOST
